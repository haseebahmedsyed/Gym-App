...
function AllOrders(props) {
    ...
    const { getAllOrders, AllOrders, DeleteOrders } = context;

    const [rowsp, setRoesp] = useState(1)
    ...
    return (
        <>
    <h1 style={{marginBottom:'47px',marginTop:'25px',textAlign: 'center'}}>Total Orders</h1>
...

                    {
                        AllOrders.map((user) => {
                            ...
                            <td rowSpan={user.Items.length}>{user.Paymentmethod}</td>
                                    ...
                                    <td rowSpan={user.Items.length} ><i onClick={() => DeleteOrders(user.ShoppingID)} class="fa fa-trash" aria-hidden="true"></i>
                                    </td>
                                </tr>
